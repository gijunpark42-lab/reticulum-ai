...
├── .env                      # ANTHROPIC_API_KEY, DART_API_KEY (gitignored)
├── .gitignore
...
python dart.py sync                     # DAILY (or on an earnings day, right after the release):
                                        #   new 잠정실적 / 공급계약 / 정기보고서 for our companies since
...
- **공급계약 →** a `contracts` entry on the edge company → 계약상대방. If that edge doesn't exist
  ...
  `[Company] DART 공급계약 (MM-DD-YYYY)` (already written in each block's header). `type` = 
  "supply agreement"; `units` = 계약기간 + 공급지역; `value` = 계약금액 in KRW + 매출액 대비 %.
